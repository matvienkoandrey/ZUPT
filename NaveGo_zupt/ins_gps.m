...
%         yawm_e(i) = hd_update (imu.mb(i,:), roll_e(i),  pitch_e(i), D);
        ...
        % ZUPT detection algorithm
        zupt = is_stance_phase( i, dti, imu.fb );
%         idz = floor( ZUPT_WINDOW / dti ); % Index to set ZUPT window time
% 
%         if ( i > idz)
%             
%             acc_m = std (imu.fb(i-idz:i , 1));
%             %acc_m = sqrt(sum(acc_m.^2));
%                          
%             if (abs(acc_m) <= ZUPT_THRELHOLD && (max(imu.fb(i-idz:i , 1))-min(imu.fb(i-idz:i , 1)))<= ZUPT_THRELHOLD-0.8)       
%                 if abs(last_zupt-i)*dti>ZUPT_DELAY
% 
